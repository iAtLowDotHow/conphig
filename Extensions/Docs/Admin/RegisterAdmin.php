...
class RegisterAdmin {
	...
	public function register_admin_dashboard() {
		...
		$page_suffix_for_documentation = add_menu_page(
			_x( 'Docs Dashboard', 'Page title', 'conphig' ),
			_x( 'Docs Page', 'Menu title', 'conphig' ),
			'manage_options',
			$primary_slug,
		...
		add_action( 'load-' . $page_suffix_for_documentation, array( $this, 'dashboard_admin_init' ) );

		// Register dashboard submenu nav item.
		add_submenu_page($primary_slug, 'Site Documentation', 'Documentation', 'manage_options', $primary_slug . '#/test', '__return_null' );
	...
	public function plugin_documentation_page() {
		require_once ADDON_DIR_DOCS. '/starlight/index.html';
	}
